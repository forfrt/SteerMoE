...
class Conv2dSubsampling(nn.Module):
    def __init__(self, idim, d_model, out_channels=32):
        logging.debug(f"idim: {idim}, d_model: {d_model}, out_channels: {out_channels}")
        super().__init__()
...
class RelPosMultiHeadAttention(EncoderMultiHeadAttention):
    def __init__(self, n_head, d_model,
        ...
        super().__init__(n_head, d_model,
                         residual_dropout)
        d_k = d_model // n_head
        logging.debug(f"n_head: {n_head}, d_model: {d_model}, residual_dropout: {residual_dropout}, d_k: {d_k}")
        # default: n_head=20, d_model=1280, d_k=64
    ...
    def _rel_shift(self, x):
        ...
        # x_padded: (3, 20, 750, 1500)

        x_padded = x_padded.view(N, H, T2 + 1, T1)
        # x_padded: (3, 20, 1500, 750)
    ...
    def forward(self, q, k, v, pos_emb, mask=None):
        ...
        # q_with_bias_u, q_with_bias_v: (3, 20, 750, 64)


        # (3, 20, 750, 64)*(3, 20, 64, 750)->(3, 20, 750, 750)
        matrix_ac = torch.matmul(q_with_bias_u, k.transpose(-2, -1))

        # (3, 20, 750, 64)*(1, 20, 64, 1499)->(3, 20, 750, 1499)
        matrix_bd = torch.matmul(q_with_bias_v, p.transpose(-2, -1))

        matrix_bd = self._rel_shift(matrix_bd)
        logging.debug(f"matrix_bd: {matrix_bd.shape}, {matrix_bd.dtype}, {matrix_bd.device}, {matrix_bd}")
        # matrix_bd: (3, 20, 750, 750)

        attn_scores = matrix_ac + matrix_bd
        attn_scores.mul_(self.scale)
        logging.debug(f"attn_scores: {attn_scores.shape}, {attn_scores.dtype}, {attn_scores.device}, {attn_scores}")

        output, attn = self.attention.forward_attention(attn_scores, v, mask=mask)
        logging.debug(f"output: {attn_scores.shape}, {attn_scores.dtype}, {attn_scores.device}, {attn_scores}")
        logging.debug(f"attn: {attn.shape}, {attn.dtype}, {attn.device}, {attn}")
